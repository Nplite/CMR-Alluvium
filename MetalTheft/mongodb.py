...
#     def __init__(self, url="mongodb://localhost:27017", db_name='MetalTheft', snapshot_collection_name='Snapshots', video_collection_name='Videos'):
#         try:
#             self.client = AsyncIOMotorClient(url, maxPoolSize=100, minPoolSize=10)
#             self.db = self.client[db_name]
#             self.snapshot_collection = self.db[snapshot_collection_name]
#             self.video_collection = self.db[video_collection_name]
#         except Exception as e:
...
#             logging.info(f"Fetched snapshots for date: {date_folder}, camera_id: {camera_id}")
#             return result['images'] if result else None
#         except Exception as e:
#             raise MetalTheptException(e, sys) from e
...
#                 }
#             ]
#             results = await self.snapshot_collection.aggregate(pipeline).to_list(length=None)
#             snapshots = [image for result in results for image in result.get('images', [])]
#             logging.info(f"Fetched {len(snapshots)} snapshots for month: {month}, year: {year}, camera_id: {camera_id}")
#             return snapshots if snapshots else None
#         except Exception as e:
#             raise MetalTheptException(e, sys) from e
