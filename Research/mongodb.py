...
#             if result.upserted_id:
#                 logging.info(f"Created new snapshot document for date: {date_folder} and camera_id: {camera_id}")
#             else:
#                 logging.info(f"Updated existing snapshot document for date: {date_folder} and camera_id: {camera_id}")
...
#             if result.upserted_id:
#                 logging.info(f"Created new video document for date: {date_folder} and camera_id: {camera_id}")
#             else:
#                 logging.info(f"Updated existing video document for date: {date_folder} and camera_id: {camera_id}")

#         except Exception as e:
#             raise MetalTheptException(e, sys) from e

#     async def fetch_snapshots_by_date_and_camera(self, year, month, day, camera_id):
#         """Fetch snapshots from MongoDB for a specific date and camera_id."""
#         try:
#             date_folder = datetime(year, month, day).strftime('%Y-%m-%d')
#             result = await self.snapshot_collection.find_one({'date': date_folder, 'camera_id': camera_id})
#             return result['images'] if result else None
#         except Exception as e:
#             raise MetalTheptException(e, sys) from e

#     async def fetch_videos_by_date_and_camera(self, year, month, day, camera_id):
#         """Fetch videos from MongoDB for a specific date and camera_id."""
#         try:
#             date_folder = datetime(year, month, day).strftime('%Y-%m-%d')
#             result = await self.video_collection.find_one({'date': date_folder, 'camera_id': camera_id})
#             return result['videos'] if result else None
#         except Exception as e:
#             raise MetalTheptException(e, sys) from e

#     async def fetch_snapshots_by_month_and_camera(self, year, month, camera_id):
#         """Fetch snapshots from MongoDB for a specific month and camera_id."""
#         try:
#             start_date = datetime(year, month, 1)
...
#                 snapshots.extend(doc['images'])
#             return snapshots if snapshots else None
#         except Exception as e:
#             raise MetalTheptException(e, sys) from e

#     async def fetch_videos_by_month_and_camera(self, year, month, camera_id):
#         """Fetch videos from MongoDB for a specific month and camera_id."""
#         try:
#             start_date = datetime(year, month, 1)
#             end_date = datetime(year, month + 1, 1) - timedelta(days=1) if month < 12 else datetime(year + 1, 1, 1) - timedelta(days=1)
...
#             videos = []
#             async for doc in cursor:
#                 videos.extend(doc['videos'])
#             return videos if videos else None
#         except Exception as e:
#             raise MetalTheptException(e, sys) from e

#     async def fetch_snapshots_with_pagination(self, year, month, day, camera_id, skip, limit):
#         """Fetch snapshots with pagination."""
#         try:
#             date_folder = datetime(year, month, day).strftime('%Y-%m-%d')
#             cursor = self.snapshot_collection.find({'date': date_folder, 'camera_id': camera_id}).skip(skip).limit(limit)
